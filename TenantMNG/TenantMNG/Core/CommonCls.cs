...
namespace TenantMNG.Core
{
    public static class CommonCls
    {
        ...
        public static string _applicationurl = "http://energymeters.xcesslogic.com/";
        ...
        public static string getMeterName(int tenantid)
        {
            ...
            if (_tenantmeter != null && _tenantmeter.Count > 0)
            {
                ...
                HISTORY_CONFIG _meter = null;

                foreach (var meterid in _tenantmeter)
                {
                    _meter = new HISTORY_CONFIG();
                    _meter = _dbmeter.HISTORY_CONFIG.Where(x => x.ID == meterid.int_meter_id).SingleOrDefault();
        ...
        //get metername for Tenant
        public static string getTenantNameforMeter(int meterid)
        {
            DB_TenantMNGEntities _dbc = new DB_TenantMNGEntities();
            var _tenantmeter = _dbc.tbl_tenant_meter.Where(x => x.int_meter_id == meterid && x.bit_is_assign == true).SingleOrDefault();
        ...
        public static string getMeterNamefromId(int? meterid)
        {
            ...
            {
                NiagaraEntities _dbmeter = new NiagaraEntities();
                var _meter = _dbmeter.HISTORY_CONFIG.Where(x => x.ID == meterid).SingleOrDefault();

                 return _meter.TABLE_NAME;
            }
            else
                return "NA";
        ...
        //check tenant have meter or not
        public static bool isMeterTenant(int id)
        {
            DB_TenantMNGEntities _dbc = new DB_TenantMNGEntities();
